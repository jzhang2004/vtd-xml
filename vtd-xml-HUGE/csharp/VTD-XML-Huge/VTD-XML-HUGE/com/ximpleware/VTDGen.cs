...
namespace com.ximpleware
{
    ...
    public class VTDGen
    {
        ...
        private int ch;
        private int ch_temp;
        protected internal long offset; // this is byte offset, not char offset as encoded in VTD
        private long temp_offset;
        protected internal int depth;


        protected internal long prev_offset;
        ...
        protected internal long docLen;
        // again, in terms of byte, not char as encoded in VTD
        protected internal long endOffset;
        protected internal long[] tag_stack;
        public long[] attr_name_array;
        public const int MAX_DEPTH = 62; // maximum depth value
        protected internal long docOffset;
        ...
        // max prefix length
        public const int MAX_PREFIX_LENGTH = (1 << 7) - 1;
        // max Qname length
        public const int MAX_QNAME_LENGTH = (1 << 10) - 1;
        // max Token length
        public const int MAX_TOKEN_LENGTH = (1 << 17) - 1;
        ...
        private void decide_encoding()
        {
            ...
            if (encoding < FORMAT_UTF_16BE)
            {
                if ((offset + docLen) >= 1L << 37)
                    throw new ParseException("Other error: file size too big >=128GB ");
            }
            else
            {
                if ((offset + docLen) >= 1L << 37)
                    throw new ParseException("Other error: file size too large >= 128GB");
            }
        }
        /// <summary>
                    ...
                    switch (parser_state)
                    {
                        ...
                        case STATE_START_TAG:  //name space is handled by
                            ...
                            if (encoding < FORMAT_UTF_16BE)
                            {
                                ...
                                writeVTD((temp_offset), (int)((length2 << 10) | length1), TOKEN_STARTING_TAG, depth);
                            }
                            else
                            {
                                if (length2 > (MAX_PREFIX_LENGTH << 1) || length1 > (MAX_QNAME_LENGTH << 1))
                                    throw new ParseException("Token Length Error: Starting tag prefix or qname length too long" + formatLineNumber());
                                writeVTD((temp_offset) >> 1, (int)((length2 << 9) | (length1 >> 1)), TOKEN_STARTING_TAG, depth);
                            }
                            //offset += length1;
                            length2 = 0;
                                    ...
                                    if (ch == '<')
                                    {
                                        ...
                                        if (r.skipChar('/'))
                                        {
                                            if (helper == true)
                                            {
                                                ...
                                                if (encoding < FORMAT_UTF_16BE)
                                                    writeVTD((temp_offset),(int) length1, TOKEN_CHARACTER_DATA, depth);
                                                else
                                                    writeVTD((temp_offset) >> 1, (int)(length1 >> 1), TOKEN_CHARACTER_DATA, depth);
                                            }
                                            parser_state = STATE_END_TAG;
                                            break;
                                        }
                        ...
                        case STATE_ATTR_NAME:
                            ...
                            for (int i = 0; i < attr_count; i++)
                            {
                                ...
                                if (length1 == prevLen)
                                {
                                    long prevOffset = attr_name_array[i]>>17 ;
                            ...
                            {
                                attr_name_array[attr_count] = (temp_offset << 17) | length1;
                                attr_count++;
                            }
                            ...
                            if (is_ns)
                            {
                                if (encoding < FORMAT_UTF_16BE)
                                {
                                    ...
                                    writeVTD(temp_offset, (int)((length2 << 10) | length1), TOKEN_ATTR_NS, depth);
                                }
                                else
                                {
                                    if (length2 > (MAX_PREFIX_LENGTH << 1) || length1 > (MAX_QNAME_LENGTH << 1))
                                        throw new ParseException("Token length overflow error: Attr NS prefix or qname length too long" + formatLineNumber());
                                    writeVTD(temp_offset >> 1, (int)((length2 << 9) | (length1 >> 1)), TOKEN_ATTR_NS, depth);
                                }
                                is_ns = false;
                            }
                            else
                            {
                                if (encoding < FORMAT_UTF_16BE)
                                {
                                    ...
                                    writeVTD(temp_offset, (int)((length2 << 10) | length1), TOKEN_ATTR_NAME, depth);
                                }
                                else
                                {
                                    if (length2 > (MAX_PREFIX_LENGTH << 1) || length1 > (MAX_QNAME_LENGTH << 1))
                                        throw new ParseException("Token Length overflow error: Attr name prefix or qname length too long" + formatLineNumber());
                                    writeVTD(temp_offset >> 1,(int)((length2 << 9) | (length1 >> 1)), TOKEN_ATTR_NAME, depth);
                                }
                            }
                            /*System.out.println(
                        ...
                        case STATE_ATTR_VAL:
                            ...
                            if (encoding < FORMAT_UTF_16BE)
                            {
                                ...
                                writeVTD(temp_offset, (int)length1, TOKEN_ATTR_VAL, depth);
                            }
                            else
                            {
                                if (length1 > (MAX_TOKEN_LENGTH << 1))
                                    throw new ParseException("Token Length Error:" + " Attr val too long (>0xfffff)" + formatLineNumber());
                                writeVTD(temp_offset >> 1, (int)(length1 >> 1), TOKEN_ATTR_VAL, depth);
                            }
                            ch = r.Char;
                            if (XMLChar.isSpaceChar(ch))
                            {
        ...
        private int process_pi_tag()
        {
            ...
            if (encoding < FORMAT_UTF_16BE)
            {
                ...
                writeVTD((temp_offset), (int)length1, TOKEN_PI_NAME, depth);
            }
            else
            {
                if (length1 > (MAX_TOKEN_LENGTH << 1))
                    throw new ParseException("Token Length Error:" + " PI name too long (>0xfffff)" + formatLineNumber());
                writeVTD((temp_offset) >> 1, (int)((length1 >> 1)), TOKEN_PI_NAME, depth);
            }
            //length1 = 0;
            temp_offset = offset;
            if (XMLChar.isSpaceChar(ch))
        ...
        private int process_pi_val()
        {
            ...
            if (encoding < FORMAT_UTF_16BE)
            {
                ...
                writeVTD(temp_offset, (int)length1, TOKEN_PI_VAL, depth);
            }
            else
            {
                if (length1 > (MAX_TOKEN_LENGTH << 1))
                    throw new ParseException("Token Length Error:" + "PI VAL too long (>0xfffff)" + formatLineNumber());
                writeVTD(temp_offset >> 1, (int)(length1 >> 1), TOKEN_PI_VAL, depth);
            }
            //length1 = 0;
            temp_offset = offset;
            ch = CharAfterSe;
        ...
        private int process_comment()
        {
            ...
            if (r.Char == '>')
            {
                ...
                if (encoding < FORMAT_UTF_16BE)
                    writeVTD(temp_offset, (int)length1, TOKEN_COMMENT, depth);
                else
                    writeVTD(temp_offset >> 1,(int)(length1 >> 1), TOKEN_COMMENT, depth);
                //length1 = 0;
                temp_offset = offset;
                ch = CharAfterSe;
                if (ch == '<')
        ...
        private int process_cdata()
        {
            ...
            if (encoding < FORMAT_UTF_16BE)
            {
                ...
                writeVTD(temp_offset, (int)length1, TOKEN_CDATA_VAL, depth);
            }
            else
            {

                writeVTD(temp_offset >> 1, (int)(length1 >> 1), TOKEN_CDATA_VAL, depth);
            }
        ...
        private int process_doc_type()
        {
            ...
            if (encoding < FORMAT_UTF_16BE)
            {
                ...
                writeVTD(temp_offset, (int)length1, TOKEN_DTD_VAL, depth);
            }
            else
            {
                if (length1 > (MAX_TOKEN_LENGTH << 1))
                    throw new ParseException("Token Length Error:" + " DTD val too long (>0xfffff)" + formatLineNumber());
                writeVTD(temp_offset >> 1, (int)(length1 >> 1), TOKEN_DTD_VAL, depth);
            }
            ch = CharAfterS;
            if (ch == '<')
            {
        ...
        /// </param>
        private void writeVTD(long offset, int length, int token_type, int depth)
        {
            //long ll;
