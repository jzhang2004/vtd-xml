...
namespace com.ximpleware
{
	...
	public class VTDNav
	{
			...
			get
			{
				...
				switch (encoding)
				{
					...
					case FORMAT_UTF_16BE: 
						...
						temp = ((XMLDoc.byteAt(currentOffset << 1)) << 8) | (XMLDoc.byteAt((currentOffset << 1) + 1));
						...
						else
						{
							...
							val = temp;
							temp = ((XMLDoc.byteAt((currentOffset << 1) + 2)) << 8) | (XMLDoc.byteAt((currentOffset << 1) + 3));
					...
					case FORMAT_UTF_16LE: 
						...
						temp = (XMLDoc.byteAt((currentOffset << 1) + 1) ) << 8 | (XMLDoc.byteAt(currentOffset << 1) );
						if (temp < 0xdc00 || temp > 0xdfff)
						{
						...
						else
						{
							...
							val = temp;
							temp = (XMLDoc.byteAt((currentOffset << 1) + 3)) << 8 | (XMLDoc.byteAt((currentOffset << 1) + 2));
							if (temp < 0xdc00 || temp > 0xdfff)
							{
			...
			get
			{
				...
				switch (ch)
				{
					...
					case 'a': 
						...
						if (ch == 'm')
						{
							if (getCharUnit(currentOffset + 1) == 'p' && getCharUnit(currentOffset + 2) == ';')
							{
								currentOffset += 3;
								val = '&';
							}
						...
						{
							if (getCharUnit(currentOffset + 1) == 'o' && getCharUnit(currentOffset + 2) == 's' && getCharUnit(currentOffset + 3) == ';')
							{
								currentOffset += 4;
								val = '\'';
							}
					...
					case 'q': 
						...
						if (getCharUnit(currentOffset) == 'u' && getCharUnit(currentOffset + 1) == 'o' && getCharUnit(currentOffset + 2) == 't' && getCharUnit(currentOffset + 3) == ';')
						{
							currentOffset += 4;
							val = '\"';
						}
					...
					case 'l': 
						if (getCharUnit(currentOffset) == 't' && getCharUnit(currentOffset + 1) == ';')
						{
							currentOffset += 2;
							val = '<';
						}
			...
			get
			{
				...
				switch (ch)
				{
					...
					case 'a': 
						...
						if (ch == 'm')
						{
							if (getCharUnit(currentOffset2 + 1) == 'p' && getCharUnit(currentOffset2 + 2) == ';')
							{
								currentOffset2 += 3;
								val = '&';
							}
						...
						{
							if (getCharUnit(currentOffset2 + 1) == 'o' && getCharUnit(currentOffset2 + 2) == 's' && getCharUnit(currentOffset2 + 3) == ';')
							{
								currentOffset2 += 4;
								val = '\'';
							}
					...
					case 'q': 
						...
						if (getCharUnit(currentOffset2) == 'u' && getCharUnit(currentOffset2 + 1) == 'o' && getCharUnit(currentOffset2 + 2) == 't' && getCharUnit(currentOffset2 + 3) == ';')
						{
							currentOffset2 += 4;
							val = '\"';
						}
					...
					case 'l': 
						if (getCharUnit(currentOffset2) == 't' && getCharUnit(currentOffset2 + 1) == ';')
						{
							currentOffset2 += 2;
							val = '<';
						}
		...
		/// <summary> Get the depth (>=0) of the current element.
		...
		private const long MASK_TOKEN_FULL_LEN = 0x000fffff00000000L;
		private const long MASK_TOKEN_PRE_LEN = 0x000ff80000000000L;
		private const long MASK_TOKEN_QN_LEN = 0x000007ff00000000L;
		...
		private const long MASK_TOKEN_TYPE = -0x7000000000000000L;
		//UPGRADE_TODO: Literal detected as an unsigned long can generate compilation errors. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1175'"
		...
		protected internal VTDNav(int RootIndex, int enc, bool NS, int depth, IByteBuffer x, ILongBuffer vtd, ILongBuffer l1, ILongBuffer l2, IIntBuffer l3, int so, int length)
		{
			...
			if (ns == false)
				MASK_TOKEN_OFFSET = unchecked((long)0xffffff7f00000000L);
			// this allows xml size to be 2GB 
			// if there is no namespace
			else
				MASK_TOKEN_OFFSET = unchecked((long)0xffffff3f00000000L);
		...
		{
			if (s == null)
				throw new System.ArgumentException("string can't be null");
			int i, l;
			...
			if (encoding < 2)
			{
				...
				{
					if (s[i] != (XMLDoc.byteAt(currentOffset)))
						return false;
					currentOffset++;
				}
				return true;
		...
		private bool matchTokenString(int offset, int len, System.String s)
		{
			...
			if (encoding < FORMAT_UTF8)
			{
				...
				for (i = 0; i < l && currentOffset < endOffset; i++)
				{
					if ((XMLDoc.byteAt(currentOffset)) != '&')
					{
						if (s[i] != (XMLDoc.byteAt(currentOffset)))
							return false;
						currentOffset++;
		...
		public double parseDouble(int index)
		{
			...
			if (ch == '-' || ch == '+')
				ch = b?CharResolved:Char; //get another one if it is sign.
			...
			if (ch == 'E' || ch == 'e')
			{
				...
				if (ch == '+' || ch == '-')
					ch = b?CharResolved:Char; //skip the +/- sign
		...
		protected internal  int parseInt(int index, int radix)
		{
			...
			int c = b?CharResolved:Char;
			
			// trim leading whitespaces
			while ((c == ' ' || c == '\n' || c == '\t' || c == '\r') && (currentOffset <= endOffset))
				c = b?CharResolved:Char;
			...
			bool neg = (c == '-');
			if (neg || c == '+')
				c = b?CharResolved:Char; //skip sign
			...
			{
				//UPGRADE_TODO: The equivalent in .NET for method 'java.lang.Character.digit' may return a different value. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1043'"
				int digit = (int)System.Char.GetNumericValue((char) c);
				if (digit < 0)
					break;
		...
		public  bool matchTokens(int i1, VTDNav vn2, int i2)
		{
			...
			if (vn2.currentOffset2 == endOffset2 && this.currentOffset == endOffset1)
				return true;
			else
				return false;
		}
