...
public class XPath {
	...
	class Predicate implements XPathNode{
		int i; // only supports a[1] style of loation path for now
		int count;
		public Predicate(){
		...
		public void setIndex(int index) throws XPathException{
			if (index<=0)
				throw new XPathException("Invalid index number");
			i = index;
		}
		public void reset(){
			count = 1;
	...
	public XPathExpr compileXPath(String s) throws XPathException{
		...
		// check for the possiblity of empty/whitespace string
		boolean bad = false;
		int i=0;
		...
		while(main_loop){
			...
			case PARSERSTATE_NODETEST:
				...
				if (ch=='('){
					...
					if (t==NodeTest.NODE && currentStep.axis_type == XPath.AXIS_ATTRIBUTE)
			...
			case PARSERSTATE_PREDICATE:
				...
				if (ch!=']')
					throw new XPathException(" Predicate not terminated properly");
				
				int temp = Integer.parseInt(s.substring(tempOffset1,tempOffset2));
				
				Predicate p1 = new Predicate();
				p1.setIndex(temp);
				System.out.println("temp--> " + temp);
				currentStep.setPredicate(p1);
			...
			case AXIS_FOLLOWING:
				...
				case XPathExpr.FORWARD:
					...
					if (currentStep.get_ft() == true) {
						...
						if (currentStep.axis_type == AXIS_PRECEDING )
							if (currentStep.nt.testType == NodeTest.NODE)
								ap.setSpecial(true);
							else
								ap.setSpecial(false);
	...
	public void dumpXPathExpr(XPathExpr xpe){
		...
		Step s = xpe.s;
		if (s == null && xpe.path_type == XPathExpr.ABSOLUTE_PATH){
