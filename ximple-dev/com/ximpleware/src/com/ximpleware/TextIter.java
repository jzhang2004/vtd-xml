 ...
 */
    ...
    private int handleLevel2(){
        ...
        if (lcLower != -1) { // at least one child element
            int temp1 = vn.l3Buffer.upper32At(lcLower);
            int temp2 = vn.l3Buffer.upper32At(lcUpper);
            lcIndex = (lcIndex != -1) ? lcIndex : lcLower;
            while (sp < vn.vtdSize) {
                int s = vn.l3Buffer.upper32At(lcIndex);
                //int s = vn.l2Buffer.upper32At(lcIndex);
                if (sp >= temp1 && sp < temp2) {
                    if (sp == s) {
                        lcIndex++;
                        sp = vn.l3Buffer.upper32At(lcIndex) - 1;
                        //boolean b = false;
    ...
    private int handleLevel3(){
        ...
        if (prevLocation != -1) {
            ...
            // fetch lclower and lcupper
            lcLower = vn.l3Buffer.lower32At(vn.l3index);
            if (lcLower != -1) {
                lcUpper = vn.l4Buffer.size - 1;
                int size = vn.l3Buffer.size;
                for (int i = vn.l3index + 1; i < size ; i++) {
                    int temp = vn.l3Buffer.lower32At(i);
                    if (temp != 0xffffffff) {
                        lcUpper = temp - 1;
                        break;
                    }
        ...
        if (lcLower != -1) { // at least one child element
            int temp1 = vn.l4Buffer.upper32At(lcLower);
            int temp2 = vn.l4Buffer.upper32At(lcUpper);
            lcIndex = (lcIndex != -1) ? lcIndex : lcLower;
            while (sp < vn.vtdSize) {
                int s = vn.l4Buffer.upper32At(lcIndex);
                //int s = vn.l2Buffer.upper32At(lcIndex);
                if (sp >= temp1 && sp < temp2) {
                    if (sp == s) {
                        lcIndex++;
                        sp = vn.l4Buffer.upper32At(lcIndex) - 1;
                        //boolean b = false;
                        while (vn.getTokenDepth(sp) == 2) {
                            sp--;
                          //  b = true;
    ...
    private int handleLevel4(){ //l2
        ...
        if (prevLocation != -1) {
            ...
            // fetch lclower and lcupper
            lcLower = vn.l4Buffer.lower32At(vn.l4index);
            if (lcLower != -1) {
                lcUpper = vn.l5Buffer.size - 1; //5
                int size = vn.l4Buffer.size; //4
                for (int i = vn.l4index + 1; i < size ; i++) {//4
                    int temp = vn.l4Buffer.lower32At(i); //4
                    if (temp != 0xffffffff) {
                        lcUpper = temp - 1;
                        break;
                    }
        ...
        if (lcLower != -1) { // at least one child element
            int temp1 = vn.l5Buffer.intAt(lcLower);
            int temp2 = vn.l5Buffer.intAt(lcUpper);
            lcIndex = (lcIndex != -1) ? lcIndex : lcLower;
            while (sp < vn.vtdSize) {
                int s = vn.l5Buffer.intAt(lcIndex);
                //int s = vn.l2Buffer.upper32At(lcIndex);
                if (sp >= temp1 && sp < temp2) {
                    if (sp == s) {
                        lcIndex++;
                        sp = vn.l5Buffer.intAt(lcIndex) - 1;
                        //boolean b = false;
